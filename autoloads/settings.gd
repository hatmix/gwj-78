...
func _ready() -> void:
	Global.controls_changed.connect(save_controls)
...
func save_controls(data: GUIDERemappingConfig) -> void:
	...
	if err:
		Global.post_ui_message.emit("Error saving controls '%s'" % str(err))
	else:
		Global.post_ui_message.emit("Controls saved")
...
func load_settings() -> void:
	...
	if err:
		Global.post_ui_message.emit("Loading default settings")
