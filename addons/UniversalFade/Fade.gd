...
static func fade_out(time := 1.0, color := Color.BLACK,  pattern: Variant = "", reverse := false, smooth := false) -> Fade:
...
static func fade_in(time := 1.0, color := Color.BLACK,  pattern: Variant = "", reverse := true, smooth := false) -> Fade:
	var fader := _create_fader(color, pattern, reverse, smooth)
...
static func crossfade_prepare(time := 1.0, pattern: Variant = "", reverse := false, smooth := false) -> void:
...
static func _create_fader(color: Color, pattern: Variant, reverse: bool, smooth: bool) -> Fade:
	...
	if pattern is Texture2D:
		texture = pattern
	elif pattern.is_empty() or not pattern is String:
		smooth = true
		reverse = false
