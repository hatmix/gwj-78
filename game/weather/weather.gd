...
const MAX_DARKNESS_PERCENT = 0.2
...
@export_range(2.0, 60.0, 1.0) var weather_change_frequency_seconds := 10.0
var _snow_intensity: float:
	set(value):
		...
		if darkener:
			darkener.color.a = value * MAX_DARKNESS_PERCENT
...
func _ready():
	...
	_snow_intensity = 1.0 if _state == State.SNOWING else 0.0
